<?php include("includes/formattop.php"); ?>
	<title>Kinito</title>
</head>

<body id="kinito" class="container project">
	<?php include("includes/navigation.php"); ?>

	<main class="row projectTitleDesc">
	
		<section class="col-xs-1 col-sm-1 col-md-2 noPadding"></section>
		<section class="col-xs-10 col-sm-10 col-md-8 noPadding">

			<h3 class="projectTitle noMargin">Linum KINITO</h3>
			<h4 class="projectCategory noMargin">Branding and package design</h4>
			<p class="projectDescription">Linum presents the Kinito hotspot phone case. With the flip of a switch, access WiFi in non-reception areas.</p>


		</section>
		<section class="col-xs-1 col-sm-1 col-md-2 noPadding"></section>
	</main>

	<main class="row projectImages">

		<section class="col-md-2 hideThis"></section>
		<section class="col-xs-12 col-md-8 noPadding images">
			
			<!-- IMG -->
			<div class="row">
				<div class="col-xs-12 noPadding">
					<img src="img/projects/kinito/kinito001.jpg">
				</div>
			</div>
			<!-- IMG -->
			<!-- IMG -->
			<div class="row">
				<div class="col-xs-12 col-sm-6 col-md-6 noPadding">
					<img src="img/projects/kinito/kinito002sketch.jpg">
				</div>
				<div class="col-xs-12 col-sm-6 col-md-6 noPadding">
					<img src="img/projects/kinito/kinito003sketch.jpg">
					<img src="img/projects/kinito/kinito004phoneVector.png">
				</div>
			</div>
			<!-- IMG -->
			<!-- IMG -->
			<div class="row">
				<div class="col-xs-12 col-sm-4 col-md-4 noPadding">
					<img src="img/projects/kinito/kinito005case.jpg">
				</div>
				<div class="col-xs-12 col-sm-8 col-md-8 noPadding">
					<img src="img/projects/kinito/kinito006case.jpg">
				</div>
			</div>
			<!-- IMG -->
			<!-- IMG -->
			<div class="row">
				<div class="col-xs-12 col-sm-6 col-md-6 noPadding">
					<img src="img/projects/kinito/kinito007outside.jpg">
				</div>
				<div class="col-xs-12 col-sm-6 col-md-6 noPadding">
					<img src="img/projects/kinito/kinito008inside.jpg">
				</div>
			</div>
			<!-- IMG -->

		</section>
		<!-- END OF .images -->
		<section class="col-md-2 hideThis"></section>
	</main>